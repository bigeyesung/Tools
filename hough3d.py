...
#
newRoot = os.path.dirname(os.path.realpath(__file__))
workspace = Path(newRoot)
inputFolder = workspace.joinpath("input")
inputFile = os.path.join(inputFolder, "Data_Wires.txt")
dataset = pd.read_csv(inputFile,sep=' ', header=None)
wirePts=dataset.to_numpy()
tmpSize = len(wirePts)
#remove class column
wirePts = wirePts[:,[0,1,2]]
#
...
merge_angle_point=0
centers=hough3(wirePts,cart,num_point_threshold,point_distance_threshold,merge_angle_point)
...
make_image(line3,k_centers,point_index,False,'/home/chenhsi/Projects/MindTrace/Pipeline/Latest/git-v-brain/V-Brain/7.png')